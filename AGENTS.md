...
| `git blame <path>` | `jj file annotate <path>`（行ごとに、その行を導入した change を表示する） |
...
- [ ] For every existing constant, threshold, guard, or branch you change, confirm how it got there (inline comment, `jj file annotate <path>`, originating PR) and state which past decision the change alters.
