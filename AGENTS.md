...
- **Preserve the working tree:** operate on the current snapshot, never discard user changes, and avoid destructive commands (`git reset --hard`, `git clean -fd`, a bare `git restore .` / `git checkout -- .`, etc.). Rewinding anything the user has not committed is available for recovery only, and only with the user's explicit approval of that specific rollback. See [Version Control (Git)](#version-control-git).
...
- Recovery for a local mistake is `git reflog` plus a rescue branch (`git branch <rescue> <sha>`) on history you own. Show the user the reflog entry you intend to move to and get explicit approval before rewinding anything. Reversibility is not a licence to run destructive commands in the first place.
