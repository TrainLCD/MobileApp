...
- **Do not hand over a hole you already found:** if you notice a case your proposal handles incorrectly, close it before proposing, or bring the resolved alternative instead. Reporting the defect as a question the user can decide later ("adjust it if it bothers you") pushes your own review work onto them.
...
- [ ] For every existing constant, threshold, guard, or branch you change, confirm how it got there (inline comment, `git log -S`, originating PR) and state which past decision the change alters.
