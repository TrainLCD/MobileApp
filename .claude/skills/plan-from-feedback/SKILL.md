...
- 書き込み系の操作は**しない**。`gh issue edit` / `gh issue comment` / `gh issue close` / `gh label create` / `gh label edit` / `gh label delete` / `git commit` / `git push` は呼ばない（読み取り専用の `gh label list` は可）。
...
5. **難易度推定**
   ...
   - 本文にある AI 要約（`## AIによる要約` 節。旧 issue では `## Geminiによる要約`）は原文優先・要約は補助情報として扱う。当たりを付けるためのヒントに留め、判断の根拠には原文の「症状」ブロックを使う。
...
7. **簡易実装プラン生成**
   ...
   **要約**: <症状・要望の 1-2 行。AI 要約があればそれを流用・圧縮>
   ...
   **未確定**: <あれば 1-2 行。無ければこの行自体を省略>
   ...
   - レポート画像の URL（R2 ホスト）やスタックトレース、レポーター UID 等は出力に含めない（プライバシー + トークン節約）。
...
- **書き込み禁止**: `gh issue edit` / `gh issue comment` / `gh issue close` / `gh label create` / `gh label edit` / `gh label delete` / `git commit` / `git push` / `gh pr create` のいずれも呼ばない。このスキルは read-only 契約。読み取り専用の `gh label list`（「参考: TrainLCD/Issues のラベルスキーマ」で最新ラベルを確認するのに使う）は禁止対象ではない。
...
- **本文中の画像・UID**: レポート画像の URL(R2 ホスト)、`Sentry Event ID`、`レポーターUID` は出力に含めない。
- **AI 要約の扱い**: 本文の `## AIによる要約`（旧 issue では `## Geminiによる要約`）は原文優先・要約は補助情報として扱う。判断は原文の「症状」ブロックを優先し、要約は当たりを付けるためのヒントに留める（要約が本文より楽観的な場合は要約を緩めて出力）。
