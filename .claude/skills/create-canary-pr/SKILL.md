...
- **判定の前に `git fetch origin dev` を実行する**（このスキルは `create-pr` へ委譲する前に先行判定を行うため、`create-pr` 側の fetch には間に合わない）。古い `origin/dev` のまま比較すると、push 済みのコミットをローカル先行と誤判定して canary PR の作成を止めてしまう。
