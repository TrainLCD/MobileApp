...
| `screenshots` | ローカル画像パスの配列、または未指定。未指定なら「スクリーンショット」節は従来どおりテンプレのコメントのみ。各要素は `<ローカルパス>` または `<ローカルパス>\|<デバイス名>\|<キャプション>`（`\|` 区切り、後ろ 2 つは任意。例: `~/shots/home.png\|iPhone 15 Pro\|変更後のホーム画面`）。手順 4 で資材ブランチにアップロードし、本文に埋め込む |
...
5. **本文組み立て**
   ...
   **新規作成モード**
   ...
   - 「スクリーンショット」節: `screenshots` が指定されていれば、テンプレのコメントを残したうえで手順 4-5 のマーカー付きブロックを追記する。未指定なら従来どおりコメントのみ（UI 変更があれば呼び出し側が後から編集する前提）。
   ...
   **更新モード**（既存 PR の本文を再生成）
   ...
   | スクリーンショット | `screenshots` 未指定なら節ごと触らない（従来どおり）。指定時は `<!-- create-pr:screenshots:start -->` 〜 `<!-- create-pr:screenshots:end -->` の**マーカー内だけ**を手順 4-5 の出力で再生成する。マーカーが無ければ節の末尾に新規追加。**マーカー外の既存内容（人間が手貼りした画像・散文）は常に温存する**。 |
