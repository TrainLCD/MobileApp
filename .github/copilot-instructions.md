...
- 復旧手段は `jj undo` と `jj op log` + `jj op restore` です。ほとんどの操作は取り消せますが、それは破壊的コマンドを試してよい理由にはなりません。
...
2. **作業コピーの保護**: 現在のスナップショットを基準に操作し、ユーザーの変更を破棄しない。破壊的なコマンド（`jj abandon`、引数なしの `jj restore`、`jj op restore`、および `git reset --hard` / `git clean -fd`）は使用しない（「バージョン管理（Jujutsu）」節を参照）
