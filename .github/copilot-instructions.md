...
- [ ] UI 変更の場合は画像を添付し、出所（端末名・React Native Web・実装のレンダリングではないイメージ図のいずれか）を明記

## バージョン管理（Git）

このリポジトリは **Git** で管理しています。バージョン管理操作は `git` と `gh` で行ってください（詳細は `AGENTS.md` の "Version Control (Git)" 節）。

- **未コミットの変更を破棄しない**: `git reset --hard` / `git clean -fd` / 引数なしの `git restore .`・`git checkout -- .` は自己判断で実行しないでください。作業を一時退避したい場合は `git stash` より WIP コミットを優先します（stash スタックは同一リポジトリの全 worktree で共有されるため、他のセッションの退避分を取り違える恐れがあります）。
- **ステージングは明示的に**: コミット前に必ず `git status` を実行して内容を目視確認し、`git add -A` で一括投入せず `git add <path>...` でコミットすべきパスだけを追加してください。
- **公開済み履歴は書き換えない**: `git push --force` は禁止です。push が弾かれたら `git fetch` して状況を見直してください。`--force-with-lease` は自分が所有するトピックブランチに限り、その push についてユーザーの明示的な承認を得た場合のみ使用できます。
- **リリースタグは annotated**（`git tag -a` / `git push origin <tag>`）。GitHub Actions 側のリリースワークフローは annotated tag を打つため、経路によってタグ種別が変わるのを避ける必要があります。
- 復旧手段は `git reflog` と、退避用ブランチの作成（`git branch <rescue> <sha>`）です。履歴を巻き戻す場合は戻したい reflog のエントリをユーザーに提示し、明示的な承認を得てから実行します。取り消せることは、破壊的コマンドを試してよい理由にはなりません。
...
2. **作業ツリーの保護**: 現在のスナップショットを基準に操作し、ユーザーの変更を破棄しない。破壊的なコマンド（`git reset --hard`、`git clean -fd`、引数なしの `git restore .` など）は使用しない。履歴の巻き戻しは明示的な承認を得た復旧時のみ例外（「バージョン管理（Git）」節を参照）
