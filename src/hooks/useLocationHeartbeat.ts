 ...
 *    iOSの'inactive'は前景として扱う。'inactive'で入るのはマルチタスク画面や通知センターを
 *    開いた間などで、アプリはまだ背景に入っていないため、上の2つはどちらも当てはまらない。
 *    ここをfalseにすると、乗車中に一度それらを開いただけでeffectが張り直され、進行中の
 *    要求が結果を残さないまま捨てられる(#6995の診断で要求だけが増え、成功・失敗・見切りが
 *    揃って0だったのがこれ)。
