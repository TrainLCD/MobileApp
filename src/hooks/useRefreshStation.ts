...
// GPS精度が極端に悪い場合に判定を見送る上限(m)
const MAX_USABLE_ACCURACY = 1000;
// GPS精度に応じた閾値補正の上限(m)
...
export const useRefreshStation = (): void => {
  ...
  const isArrived = useMemo((): boolean => {
    ...
    // GPS精度が極端に悪い場合は判定をスキップし直前の状態を維持する
    if (accuracy != null && accuracy > MAX_USABLE_ACCURACY) {
      return true;
    }
