...
export const useRefreshStation = (): void => {
  ...
  const isArrived = useMemo((): boolean => {
    ...
    // 到着圏はGPS精度のみで決める。かつては精度劣化(>200m)時にETA仮想時計が同じ駅の
    // 停車を示すなら到着圏を広げるR1(#6366)を併用していたが、Remote Configで有効化して
    // 実走させたところ到着判定がむしろ悪化したため撤去した(R1は最大500mまで到着圏を
    // 広げるので、ETAが実際の運行とずれた区間では駅から遠い地点で到着が確定する)。
    // ETAは測位の棄却(#6939)にのみ使い、到着判定を広げる用途には使わない。
