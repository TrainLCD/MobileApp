 ...
 * その後、到着しきい値を緩和する R1 も実走で到着判定を悪化させたため廃止したので、
 * 現在の ETA の役割は「明らかに行き過ぎた測位を棄却する」ことだけに限定されている:
 * - setLocation(store/atoms/location.ts) が getEtaPhaseNow で仮想時計フェーズをオンデマンド
 *   計算し、ETAが許す進行量を超えた測位を受理しない(#6939)。到着・接近・現在地の判定は
 *   常にGPSが行い、ETAがこれらを広げたり進めたりすることはない。
