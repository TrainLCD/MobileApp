...
const useTTSText = (
	...
	const nextStationNumberText = useMemo(() => {
		...
		return `${
			nextStationNumber.lineSymbol.length > 0 || theme === APP_THEME.JR_WEST
				? ""
				: "Station Number "
		}${symbol} ${num}.`;
			...
			const map = {
				[APP_THEME.TOKYO_METRO]: {
					NEXT: `The next stop is ${nextStation?.nameRoman}${
						nextStationNumberText.length > 0 ? ` ${nextStationNumberText}` : "."
					}${
						transferLines.length > 0
				...
				[APP_THEME.JR_WEST]: {
					NEXT: `${
						...
						nextStationNumber?.lineSymbol.length > 0
