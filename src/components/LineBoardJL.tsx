...
const LineBoardJL: React.FC<Props> = ({ stations, lineColors }: Props) => {
  ...
  const getLeft = useCallback((index: number) => {
    if (isTablet) {
      return barWidth * (index + 1) - barWidth / 1.75;
    }
    return barWidth * (index + 1) - barWidth;
  }, []);
            ...
            {getIsPass(stations[i]) ? (
              ...
              <View
                style={{
                  ...
                  left: getLeft(i),
                  bottom: getBottom(i),
                  width: i <= currentStationIndex ? 48 : 48,
                  height: i <= currentStationIndex ? 48 : 48,
                }}
              />
            )}
          </React.Fragment>
...
export default React.memo(LineBoardJL);
