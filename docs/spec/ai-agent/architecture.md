...
- `locale`: `ja` | `en`。応答言語の指示に使う。クライアントは端末の言語設定
  ではなくユーザの入力言語から決めて送る（「多言語対応」を参照）。
...
- UI 文言は `assets/translations/ja.json` / `en.json` にキー追加、
  `translate()` で参照。
- エージェント応答言語はリクエストの `locale` で指示。値は端末の言語設定
  （`isJapanese`）ではなく、ユーザが実際に書いた言語から決める
  （`src/utils/agentLocale.ts` の `resolveAgentLocale`）。英語端末で日本語を
  打ったら日本語で返ってくるようにするため。
- 判定は送信直前の（切り詰め後の）履歴に対して行い、最新のユーザ発話を
  優先する。手がかりの強い順に、
  1. ひらがながあれば `ja`（英文に混ざる日本語はカタカナ・漢字の固有名詞で、
     ひらがなは助詞・活用として現れるため）
  2. 英単語が 2 語以上あれば `en`（`I want to go to 鎌倉` は英語）
  3. 日本語文字だけなら `ja`
  4. どれにも当たらない短い発話（`Kamakura` だけ、`OK`、数字のみ）は判定不能
- 判定不能な発話は直近の判定できるユーザ発話へさかのぼり、それも無ければ
  端末の言語設定（`isJapanese`）へフォールバックする。
