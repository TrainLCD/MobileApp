...
- PoC の LLM モデルは GPT もしくは Claude を検討し、合理的な価格帯と
  返答の精度で検証する（#6473 非機能要件。検証方法は「モデル選定」参照）。
...
- `locale`: `ja` | `en`。端末の言語設定。応答言語は直近のユーザ発話の言語に
  合わせるため、これは入力言語を判別できないときの既定言語のヒントと、
  サーバ生成の定型文の言語選択に使う（「多言語対応」を参照）。
...
Claude（Anthropic）を候補とし、価格と精度で検証する。定性比較:
...
比較検証の結果で決める。Claude 側の候補は次の 2 モデル:
...
GPT 側の料金は改定が頻繁なため本書には記載せず、検証時に公式料金表で
確認して同条件で比較する。モデル名も世代交代が早いため、検証時点で
最新世代の同ティアに読み替える。
...
`reply` の言語はシステムプロンプトの出力規約で「直近のユーザ発話と同じ
言語で書く」と指示する。判別できない発話（駅名だけ・`OK` だけ など）は
リクエストの `locale` を既定言語として使う。詳細は「多言語対応」を参照。
...
- クライアントは、サーバの期限超過を 30 秒の自前タイムアウトと同じ
  `timeout` として扱う。対象はストリーム開始前の 504 と、`code` が
  `deadline-exceeded` の `error` イベントの 2 つ。
...
- 推奨: Vercel AI SDK（`ai`）を薄い抽象化層として採用する。
  理由は次の 3 点。
  1. GPT / Claude を同一コードで比較検証でき、モデル選定の評価と
...
| プロバイダ | 採用 | `@ai-sdk/anthropic` / `@ai-sdk/openai` | 比較検証に両対応 |
...
前提: 1 ターン = ゲート 1 回 + 本体 LLM 呼び出し。通常ケースは
LLM 2 回（ツールループ 1 往復）、最大ケースはループ上限到達時の
LLM 4 回（3 イテレーション + 最終応答。ツール結果の蓄積と並列呼び出し
の分だけ入力トークンも増える）。通常ケースの入力はシステムプロンプト
約 2,000 tok（2 回目以降キャッシュ読み取り）+ 履歴・ツール結果約
3,000 tok、出力約 400 tok。

| 項目 | `claude-opus-5` | `claude-haiku-4-5` |
| --- | --- | --- |
| 1 ターン概算（通常） | 約 $0.04〜0.06 | 約 $0.008〜0.012 |
| 1 ターン概算（最大） | 約 $0.09〜0.12 | 約 $0.018〜0.024 |
| 1,000 ターン/日（通常） | 約 $40〜60/日 | 約 $8〜12/日 |
| 謝絶リクエスト | ほぼ $0 | ほぼ $0 |
| AI Gateway 追加費用 | $0 | $0 |
...
- トピックゲートにより無関係リクエストのコストはほぼゼロに抑えられる
  （MUST 要件 5 の狙いどおり）。
- AI Gateway はコア機能（プロキシ・分析・キャッシュ・レート制限）が
  無料で、通過リクエストへの従量課金はない。ログ保存枠は無料プランで
  アカウント全体 10 万件/月（他用途のログと共有のため、想定ログ量
  約 6〜12 万件/月が収まる保証はない）、Workers Paid プランでは
  ゲートウェイあたり 1,000 万件。想定量は Workers Paid の枠に収まる
  ことを前提とする。課金対象は Logpush・ガードレール・Unified Billing
  （プロバイダ料金の Cloudflare 経由決済、手数料あり）を使う場合のみ
  で、本設計は LLM API キーを自前管理・本文ログ無効化のため対象外。
- レート制限（30 ターン/日/人）が総量の天井になる。
- GPT 候補の試算は検証時の公式料金で同条件計算する。
...
- エントリポイントの配置や画面レイアウト・コンポーネント構成などの
  UI 設計は本書のスコープ外。本書はデータフロー・状態管理・API 接続
  のみを規定する。
...
- `rateLimited` は確定情報、`timeout` は既に 30 秒（サーバの期限超過なら
  25 秒）待たせているため、どちらもフォールバックしない。サーバの期限超過を
  再送すると、同じ処理をもう一度待たせることになる。
...
- UI 文言は `assets/translations/ja.json` / `en.json` にキー追加、
  `translate()` で参照。
- エージェント応答言語は**直近のユーザ発話と同じ言語**に合わせる。英語端末で
  日本語を打てば日本語で返る。指示はシステムプロンプトの出力規約で行い、
  判定はモデルに任せる（クライアントは言語判定をしない）。
- リクエストの `locale` は端末の言語設定（`isJapanese`）から導出して送り、
  用途は次の 2 つに限る。
  1. モデルが入力言語を判別できないとき（駅名だけ・`OK` だけ など）の
     既定言語のヒント
  2. サーバが生成する定型文（トピックゲートの謝絶文、モデル出力が空だった
     ときのフォールバック文）の言語選択。これらはモデルを通らないため
     `locale` で決まる
- 429（レート制限）の文言は上記に含まれない。サーバの 429 応答本文は機械可読な
  エラーメッセージのみで、ユーザに見せる文言はクライアントが
  `translate('destinationAgentRateLimited')` で表示する（端末の言語設定に従う）。
...
| サーバの期限超過（504 / `deadline-exceeded`） | `timeout` 扱い。再送しない |
...
- AI Gateway のログ: Cloudflare の既定では本文ペイロードも保存される
  ため、全リクエストに `cf-aig-collect-log-payload: false` を付与
  （または Gateway 側設定で本文保存を無効化）し、メタデータのみを
  記録する。本文ログを有効化する場合は dev 環境に限定し、会話ログの
  保存方針の判断に従う。
...
- トレーシング: Vercel AI SDK の呼び出しを LangSmith のトレース連携で
  ...
  「セキュリティ・プライバシー」参照）。
- 評価: モデル比較検証の評価セットを LangSmith の Dataset に登録し、
