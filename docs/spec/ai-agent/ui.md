...
- ステータス: ドラフト（@TinyKitten が取捨選択して Figma に落とし込むための中間計画書）
...
各節の複数案は @TinyKitten の取捨選択を想定した提示であり、
Figma 化の際に採用案を確定する。推奨案には（推奨）を付す。
...
採否メモ: PoC は案 A のみ。Phase 1 の利用率を見て案 B を判断する
（本書末尾「未決事項」参照）。
...
- 補助表示（任意・要取捨選択）: タイトル下に現在駅を小さく表示
  （`現在地: 高円寺` fontSize 12 / opacity 0.6）。リクエストの
  `currentStationGroupId` に何が使われるかをユーザへ可視化する意図。
...
- 選択済み表現: `SelectBoundModal` で方向確定に至った提案カードは
  `checked`（`CommonCard` の既存 prop）を付けて再訪時に区別する
  （任意・要取捨選択）。
...
- サーバ側タイムアウトは最大 25 秒 + クライアント 30 秒
  （architecture.md）。10 秒経過で補助文言
  `考えています…しばらくお待ちください` をドット下に追加表示する
  （任意）。
...
- 入力仕様:
  ...
  - 送信条件: 空白のみは不可。送信中（タイピングインジケータ表示中）
    は送信ボタンを `opacity: 0.5` で無効化（連投防止 =
    ステートレス API の会話整合性を守る）。
...
| --- | --- | --- |
| 画面背景 | `#FAFAFA` | `#212121` |
| バブル | 白カード + シャドウ / `#008ffe` | `#212121` / `#333` + 白ボーダー、角丸 0 |
...
- `destinationAgentThinkingSlow`
  - ja: 考えています…しばらくお待ちください
  - en: Still thinking… hang tight
...
- コントラスト: ユーザバブル `#008ffe` 上の白文字は WCAG AA
  （通常テキスト）を満たさないため、Figma 段階で `#0071CE` 等への
  暗色化を検討する（既存 `Button` と同色のためアプリ全体の色改定と
  合わせて判断。未決事項参照）。
...
## Figma 落とし込みチェックリスト（@TinyKitten 向け）

取捨選択が必要な判断点の一覧。

- [ ] エントリポイント: 案 A（検索バー直下バナー）で確定するか
- [ ] ヘッダーの現在駅補助表示を入れるか
- [ ] バブルの「しっぽ」表現の有無
- [ ] 提案カードの `checked` 表示（選択済み区別）を入れるか
- [ ] タイピングインジケータの 10 秒経過補助文言を入れるか
- [ ] 例文チップの文言 3 件の最終稿
- [ ] ユーザバブル色のコントラスト対応（`#008ffe` のまま / 暗色化）
- [ ] LED テーマの各要素の質感（特にバブルのボーダー表現）
- [ ] 注意書き（免責）の常設化の要否（空状態のみ → 入力バー上に常設）

## 未決事項（UI）

1. **エントリポイントの最終形**: PoC は案 A のみで開始し、Phase 1 の
   利用率次第で案 B（タブ化）を再検討する方針で良いか。
2. **ユーザバブルの色**: 既存アクセント `#008ffe` はコントラスト比が
   AA に届かない。チャット文脈だけ暗色にするか、アプリ全体の
   アクセント色改定として扱うか。
3. **会話リセットの位置**: ヘッダー右で良いか（誤タップ懸念があれば
   メニュー化 or 長押し化）。
4. **提案カード再取得（`GET_STATIONS_BY_IDS`）のタイミング**: 受信時
   一括取得（推奨）で確定して良いか。応答体感への影響が問題になれば
   タップ時取得へ変更する余地を残す。
