...
| 候補 | 評価 |
| --- | --- |
| Vercel AI SDK（`ai`） | 採用推奨。プロバイダ非依存で Workers 対応 |
| 各社公式 SDK の直接利用 | 依存最小だがプロバイダ切替を自前実装 |
| LangChain / Mastra 等 | 単一エージェント・ツール 1 本には過剰 |

- 推奨: Vercel AI SDK を薄い抽象化層として採用する。理由は次の 3 点。
