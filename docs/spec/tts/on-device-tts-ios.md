...
**辞書にも規則にも載らない語が 1 つでもあれば、その回は合成せずエラーを返す**（JS 側が端末内蔵
TTS へ倒す）。読み飛ばすと駅名が丸ごと欠けたアナウンスが無自覚に流れうるため、音質より
欠落させないことを優先している。
...
- 読めない語でフォールバックする頻度。ネイティブが返す `unreadable_text` のエラーメッセージに
  読めなかった語が入るので、実路線を流して確認する
