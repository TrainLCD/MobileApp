...
判断も PR も残らずに終わったときは、どこで止まったのかを issue にコメントして
からジョブを落とします。無言で終わると、フィードバックを見る人からは「自動修正
が調べたうえで何もしなかった」のか「そもそも動かなかった」のかが区別できず、
未処理の issue が処理済みに見えてしまいます。

内訳は次の 5 つから選ばれます。文面は固定で、エージェントの自由記述ではありません。
...
どれもコメントの目印は `<!-- auto-fix-from-feedback:error -->` で、判断のときの
目印とは別です。重複検知には引っかからないため、一過性の失敗なら次にラベルが
付いたときや手動実行でもう一度試せます。

判断を残したあとに実行が落ちた場合は、判断の内容と「実行自体は完了していない」
ことを両方書きます。片方だけではどちらも誤解になります。

なお、ラベル条件を満たしていない場合や、既に PR・ブランチ・判断済みコメントが
ある場合はコメントしません。トリアージの対象外なので、issue に書くことが
ありません。
